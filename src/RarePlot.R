RarePlot <- function(file,groups,ylab= "Number of Different OTUs",xlab= "Number of Tags Sampled",pch= NA, xlim = NULL, ylim = NULL, error=FALSE) #Same Defaults as matplot
{
  ...
  if (error == FALSE)
  {
    ...
    Number <- TrueDat[1]
    Columns <- TrueDat[-1]
    matplot(Number,Columns[,groups],ylab= ylab,xlab = xlab, pch=pch,col=1:15, xlim = xlim, ylim = ylim)
    legend("topleft",legend=names(Columns[,groups]), col =1:15,pch=19)
    matlines(Number,Columns[,groups], col=1:15)
  }
  else 
  {
